...
function mockDispatch (opts, handler) {
  ...
  const handleMockDispatchWithData = (mockDispatch) => {
  ...
  function handleDispatchCallbackPromise (dispatchCallbackPromise) {
    dispatchCallbackPromise.then((result) => {
      mockDispatch.data = {
        ...mockDispatch.data,
        ...result
      }

      handleMockDispatchWithData(mockDispatch)
    })
      .catch((err) => {
        handler.onError(err)
      })
  ...
  if (mockDispatch.data.callback) {
    const dispatchCallbackResult = mockDispatch.data.callback(opts)

    if (isPromise(dispatchCallbackResult)) {
      handleDispatchCallbackPromise(dispatchCallbackResult)
      return
    }
